...
class CVLStyleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # get the images
        img_pos = self.data[positive][0].convert("RGB")
        img_neg = self.data[negative][0].convert("RGB")
        ...
        img_neg = sizefix(img_neg)

        fheight, fwidth = self.fixed_size[0], self.fixed_size[1]

        def resize_glitch(zz):
            if self.subset == "train":
                nwidth = int(np.random.uniform(0.75, 1.25) * zz.width)
                nheight = int(
                    (np.random.uniform(0.9, 1.1) * zz.height / zz.width) * nwidth
                )
            else:
                nwidth = zz.width
                nheight = zz.height
            ...
            zz = image_resize_PIL(
                zz, height=int(1.0 * nheight), width=int(1.0 * nwidth)
            )
            zz = centered_PIL(zz, (fheight, fwidth), border_value=255.0)
            return zz

        img = resize_glitch(img)
        img_pos = resize_glitch(img_pos)
        img_neg = resize_glitch(img_neg)
